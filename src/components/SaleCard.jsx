        ...
        <>
            <div className='product-card pointer  stylist-card position-relative p-0'>
                <div className='position-relative'>
                    <img src={ props.path + props.img._id + "/" + props.img.product_images[0].file_name} alt={props.img.name} className='w-100' onClick={() => navigate("/Product-info")} />
                    <Button className='add-to-card-btn' onClick={ () => handleShow(props.img._id)}>Add to Cart</Button>
                </div>
                <Button className='discount'><RiFlashlightFill /><p style={{ fontSize: "13px", fontWeight: "600" }}> -{props.img.discount_percentage}%</p></Button>
                <div className='desc' onClick={() => navigate("/Product-info")}>
